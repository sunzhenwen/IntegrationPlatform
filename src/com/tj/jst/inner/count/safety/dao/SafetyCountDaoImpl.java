package com.tj.jst.inner.count.safety.dao;
 ...
 *
 */
public class SafetyCountDaoImpl extends BaseDao implements ISafetyCountDao {
	 ...
	 * 其他
	 */
	public List<Integer> queryOthers(String checkNum, String check,
			String startDate, String endDate) {
		...
		try {
			...
			if (check.equals("2")) {
				sql = "select count(*) as cityCount from TB_SAFETYSUPERVISION where apptype=1 and prjNum in (select distinct prjNum from TBPROJECTINFO_S where citynum ='"
						+ checkNum.trim()
						+ "' and prjTypeNum='99')"
						+ sb.toString();

			} else {
				sql = "select count(*) as cityCount from TB_SAFETYSUPERVISION where apptype=1 and prjNum in (select distinct prjNum from TBPROJECTINFO_S where countynum ='"
						+ checkNum.trim()
						+ "' and prjTypeNum='99')"
						+ sb.toString();
			}
	 ...
	 * 市政工程
	 */
	public List<Integer> queryMunicipals(String checkNum, String check,
			String startDate, String endDate) {
	 ...
	 * 查询房屋建筑工程
	 */
	public List<Integer> queryHouses(String checkNum, String check,
			String startDate, String endDate) {
	 ...
	 * 其他
	 */
	public List<Integer> queryOthers(List<DictionaryClass> citys,
			String startDate, String endDate) {
		...
		try {
			...
			for (int i = 0; i < citys.size(); i++) {
				sql = "select count(*) as cityCount from TB_SAFETYSUPERVISION where apptype=1 and prjNum in (select distinct prjNum from TBPROJECTINFO_S where citynum ='"
						+ citys.get(i).getCode().trim()
						+ "' and prjTypeNum='99')" + sb.toString();
				pstmt = conn.prepareStatement(sql);
				rs = pstmt.executeQuery();
				while (rs.next()) {
					others.add(rs.getInt("cityCount"));
	 ...
	 * 市政工程
	 */
	public List<Integer> queryMunicipals(List<DictionaryClass> citys,
			String startDate, String endDate) {
	 ...
	 * 查询房屋建筑工程
	 */
	public List<Integer> queryHouses(List<DictionaryClass> citys,
			String startDate, String endDate) {
	...
	public List<ConstructionStatistics> queryPie(String startDate,String endDate)
	{
		...
		try {
			...
			sql = "select count(*) as cityCount,cityName from v_tb_safety t where apptype=1 and provinceNum='230000' group by cityName "+sb.toString();
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while(rs.next())
			{
	...
	public List<ConstructionStatistics> queryCityPie(String checkNum,String check,String startDate,String endDate)
	{
		...
		try {
			...
			//分组查询房屋建筑工程、市政工程、其它
			sql = "select count(*) as cityCount,prjTypeNum from v_tb_safety t where apptype=1 and citynum='"+checkNum.trim()+"' group by prjTypeNum "+sb.toString();
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while(rs.next())
			{
	...
	public List<ConstructionStatistics> queryAreaPie(String checkNum,String check,String startDate,String endDate)
	{
		...
		try {
			...
			//分组查询房屋建筑工程、市政工程、其它
			sql = "select count(*) as cityCount,prjTypeNum from v_tb_safety t where apptype=1 and countynum='"+checkNum.trim()+"' group by prjTypeNum "+sb.toString();
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while(rs.next())
			{
