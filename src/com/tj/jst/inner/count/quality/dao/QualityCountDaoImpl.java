package com.tj.jst.inner.count.quality.dao;
 ...
 *
 */
public class QualityCountDaoImpl extends BaseDao implements IQualityCountDao {
	...
	public List<Integer> queryOthers(String checkNum, String check,
		...
		try {
			...
			if (check.equals("2")) {
				sql = "select count(*) as cityCount from TB_QUALITYSUPERVISION where apptype=1 and prjNum in (select distinct prjNum from TBPROJECTINFO_S where citynum ='"
						+ checkNum.trim()
						+ "' and prjTypeNum='99')"
						+ sb.toString();

			} else {
				sql = "select count(*) as cityCount from TB_QUALITYSUPERVISION where apptype=1 and prjNum in (select distinct prjNum from TBPROJECTINFO_S where countynum ='"
						+ checkNum.trim()
						+ "' and prjTypeNum='99')"
						+ sb.toString();
			}
	...
	public List<ConstructionStatistics> queryPie(String startDate,String endDate)
	{
		...
		try {
			...
			sql = "select count(*) as cityCount,cityName from v_tb_qu t where apptype=1 and provinceNum='230000' group by cityName "+sb.toString();
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while(rs.next())
			{
	...
	public List<ConstructionStatistics> queryCityPie(String checkNum,String check,String startDate,String endDate)
	{
		...
		try {
			...
			//分组查询房屋建筑工程、市政工程、其它
			sql = "select count(*) as cityCount,prjTypeNum from v_tb_qu t where apptype=1 and citynum='"+checkNum.trim()+"' group by prjTypeNum "+sb.toString();
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while(rs.next())
			{
	...
	public List<ConstructionStatistics> queryAreaPie(String checkNum,String check,String startDate,String endDate)
	{
		...
		try {
			...
			//分组查询房屋建筑工程、市政工程、其它
			sql = "select count(*) as cityCount,prjTypeNum from v_tb_qu t where apptype=1 and countynum='"+checkNum.trim()+"' group by prjTypeNum "+sb.toString();
			pstmt = conn.prepareStatement(sql);
			rs = pstmt.executeQuery();
			while(rs.next())
			{
