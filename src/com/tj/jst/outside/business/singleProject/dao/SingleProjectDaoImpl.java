package com.tj.jst.outside.business.singleProject.dao;
 ...
 *
 */
public class SingleProjectDaoImpl extends BaseDao implements ISingleProjectDao {
	 ...
	 * @return
	 */
	public void add(SingleProject sp)
	{
		int spId = 0;
		...
		try {
			...
			String orderSql = "select max(unitid) as unitid from TBPROJECTUNITINFO ";
			...
			if(rs.next())
			{
				spId = rs.getInt("unitid");//主键    SINGLE_SEQ.Nextval
				spId = spId + 1;
				...
				pstmt.setInt(1, spId);
				pstmt.setString(2,sp.getPrjId());//主体项目主键
				pstmt.setString(3,sp.getSpNum());//单位编码
				pstmt.setString(4,sp.getBuildingName());//单位建(构)筑物名称
				pstmt.setString(5,sp.getBuildingArea());//建筑面积(平方米)
				pstmt.setString(6,sp.getInvestAmount());//投资额(万元)
				pstmt.setString(7,sp.getOvergroundArea());//地上建筑面积(平方米)
				pstmt.setString(8,sp.getUndergroundArea());//地下建筑面积(平方米)
				pstmt.setInt(9, sp.getOvergroundNum());//地上层数
				pstmt.setInt(10, sp.getUndergroundNum());//地下层数
				pstmt.setString(11,sp.getBuildingHeight());//建筑高度(米)
				pstmt.setString(12,sp.getEngineeringGrade());//工程等级   0:大型 1:中型  2:小型 
				pstmt.setString(13,sp.getLengthKm());//长度(公里)
				pstmt.setString(14,sp.getSpanM());//跨度(米)
				pstmt.setString(15,sp.getPrjStructureTypeNum());//结构体系(TBPRJSTRUCTURETYPEDIC)
				pstmt.setString(16,sp.getProjectSize());//工程规模
				pstmt.setString(17,sp.getOther());//其它
				pstmt.setString(18,sp.getLinkName());//环节名称
				pstmt.executeUpdate();
			}
			conn.commit();
		} catch (SQLException e) {
	 ...
	 * @return
	 */
	public void modify(SingleProject sp)
	{
