...
<script type="text/javascript">
	...
	function queryTitle()
	{
		document.form1.action="<%=basepath%>/permitCountAction_queryPage.action";
	...
	$(function () {
		...
		$('#container').highcharts({
	        ...
	        title: {
	            text: '<s:property value="title"/>施工许可区域分布统计'
	        },
	        subtitle: {
	...
	function queryArea() {
		$(document).ready(
				function() {
					...
					$.ajax({
						url : 'permitCountAction_queryArea.action',
						type : 'post',
						dataType : 'json',
						data : 'cityId=' + cityId,
			...
			<form>
				<div class="ry_main" cellpadding="0" cellspacing="0">
					...
					<table class="ry_left left">
						...
						<tr>
							<td>所属地市：</td>
							<td><select id="cityId" name="cityNum"
								onchange="queryArea()">
									<option value="">请选择</option>
									<s:iterator value="citys">
										<option value="<s:property value="code"/>"
											<s:if test='cityNum==code'>selected</s:if>><s:property
												value="name" /></option>
									</s:iterator>
							</select></td>
							<td>所属区县：</td>
							<td><select id="areaId" name="areaNum" class="required">
									<option value="">请选择</option>
							</select></td>
						</tr>
