...
<script type="text/javascript">
	 		...
	 		submitHandler:function(form){
	            var check = /^([1-9]\d*|0)(\.\d*[1-9])?$/; 
	 			...
	 			}else
	 			{
		 			...
		 			{
	 					$("#form1").attr("action", "<%=basepath %>/designContractFilingAction_modify.action");
	...
	function goBack()
	{
		document.form1.action="<%=basepath %>/designContractFilingAction_goList.action";
	...
	function queryTenderInfo()
	{
		...
		var url="<%=basepath %>/designTenderInfoAction_queryPage.action?condition.prjNum="+prjNum;
	...
	function queryTitleLeve()
	{
		...
		var url2="<%=basepath %>/queryDesignTitleLeveAction_queryPage.action?condition.tenderCorpCode="+tenderCorpCode;
	...
	function upload(count,id,accessotyId,linkName)
	{
		...
		if(uf!="")
		{
			...
			var actionUrl = "<%=basepath %>/fileUpload_uploadFile.action?id="+id+"&accessotyId="+accessotyId+"&linkName="+linkName+"&returnRoot=designContractFiling";
	...
	{
		document.form1.action="<%=basepath %>/fileUpload_deleteFile.action?id="+uploadId+"&returnRoot=designContractFiling";
		document.form1.submit();
	}
	//下载
	function downLoad(uploadId)
	{
		document.form1.action="<%=basepath %>/fileUpload_downLoadFile.action?id="+uploadId+"&returnRoot=designContractFiling";
		document.form1.submit();
	}
</script>
