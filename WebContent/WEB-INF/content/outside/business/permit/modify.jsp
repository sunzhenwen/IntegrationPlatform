...
<script type="text/javascript">
	...
	function upload(count,id,accessotyId,linkName)
	{
		...
		if(uf!="")
		{
			...
			var actionUrl = "<%=basepath %>/fileUpload_uploadFile.action?id="+id+"&accessotyId="+accessotyId+"&linkName="+linkName+"&returnRoot=permitSuccess";
	...
	{ 
		var prjNum = $("#prjNum").val();
		var prjId = $("#prjId").val();
		var url = "<%=basepath %>/singleProjectAction_queryPage.action?prjNum="+prjNum+"&prjId="+prjId+"&condition.prjId="+prjId+"&condition.linkName=5&linkName=5";
                ...
                <table class="" cellpadding="0" cellspacing="0">
                	<tbody>   
                        ...
                        <tr>
                        	<th>施工许可证编号：</th>
							<td colspan="4" align="left">
								<input type="text" size="110" readonly name="permit.builderLicenceNum" value="<s:property value="permit.builderLicenceNum"/>" class="required"/><strong><font color="#FF0000">*</font></strong>
							</td>
                         </tr>
                         <tr>
                         	<th>建设用地规划许可证编号：</th>
                         ...
                         <tr>
                         	<th>合同开工日期：</th>
							<td align="left">
							<input id="time1" name="permit.starDate" onchange="checkDate()" value="<s:date name="permit.starDate" format="yyyy-MM-dd" />" type="text" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd',alwaysUseStartDate:true,isShowClear:true,readOnly:true})" />
							</td>
							<th>合同竣工日期：</th>
							<td align="left">
							<input id="time2" name="permit.endDate" onchange="checkDate()" value="<s:date name="permit.endDate" format="yyyy-MM-dd" />" type="text" onFocus="WdatePicker({dateFmt:'yyyy-MM-dd',alwaysUseStartDate:true,isShowClear:true,readOnly:true})" />
							</td>
						...
						<s:iterator status="i" value="permit.accessoryList">
							<tr>
							...
							&nbsp;
							<input type="button" onclick="upload('<s:property value="#i.count"/>','<s:property value="permit.permitId"/>','<s:property value="id"/>','<s:property value="linkName"/>')" value="上传" />
							&nbsp;<br />
